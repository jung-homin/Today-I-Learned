...
- **F.I.R.S.T** _(p.167)_
  ...
  - **반복가능하게*Repeatable*:** 테스트는 어떤 환경에서도 반복 가능해야 한다. 실제 환경, QA환경, 버스를 타고 집으로 가는 길에 사용하는 (네트워크에 연결되지 않은) 노트북 환경에서도 실행할 수 있어야 한다.

  - **자가검증하는*Self-Vaildating*:** 테스트는 부울*bool* 값으로 결과를 내야 한다. 성공 아니면 실패다.
...
그동안 작고 귀여운 프로젝트들을 진행하면서 테스트라는 것은 생각하지 못했다. 아마 매우매우 작은 프로젝트였기에 그랬으리라 생각한다. 실제로 내 영역에서 테스트가 필요한 작업을 많이 할 일도 없기 때문에 더 그렇다. HTML, CSS가 주이기 때문에... 이번 클린코드를 읽는동안 정말 "개발"이 하고 싶어졌다. 그냥 퍼블리싱만이 아닌 "개발"을...
...
- **클래스는 작아야 한다!**
  ...
  - 함수는 물리적인 행 수로 크기를 측정했다. 클래스는 다른 척도를 사용한다. 클래스가 맡은 책임을 샌다. _(p.173)_

  - 클래스 이름에 Processor, Manager, Super 등과 같이 모호한 단어가 있다면 클래스에다 여러 책임을 떠안겼다는 증거다. _(p.175)_

```
😀 소감
```

처음에 Javascript를 배울 때 물론 몇 달이 채 되지 않았지만... 클래스라는 개념이 정말 이해가 안됐다. 붕어빵 틀이니 뭐니 책이나, 인터넷에서 다양한 예를 들어가며 설명을 해줬었는데 그래도 이해가 안됐다... 지금은 어떤가? 그나마... 아주 그나마 기초적인 개념은 이해하고 있다고 생각한다. 그렇지만 역시 많이 부족하겠지... 사실 직접 코딩을 하면서 클래스를 만들 일이 거의 없다 싶었으니까... 아직 한참 바닥이구나 올라갈 곳이 아주 많이 남아있어서 좋다!
